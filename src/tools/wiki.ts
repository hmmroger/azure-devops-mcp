...
function configureWikiTools(server: McpServer, tokenProvider: () => Promise<AccessToken>, connectionProvider: () => Promise<WebApi>, disabledTools: Set<string>) {
  if (!disabledTools.has(WIKI_TOOLS.get_wiki)) {
    server.tool(
      WIKI_TOOLS.get_wiki,
  ...
  if (!disabledTools.has(WIKI_TOOLS.list_wikis)) {
    server.tool(
  ...
  if (!disabledTools.has(WIKI_TOOLS.list_wiki_pages)) {
    server.tool(
  ...
  if (!disabledTools.has(WIKI_TOOLS.get_wiki_page_content)) {
    server.tool(
