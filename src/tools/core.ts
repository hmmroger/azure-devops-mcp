...
import type { ProjectInfo, TeamProjectReference } from "azure-devops-node-api/interfaces/CoreInterfaces.js";
import { IdentityBase } from "azure-devops-node-api/interfaces/IdentitiesInterfaces.js";

export type TeamProjectResult = Pick<TeamProjectReference, "id" | "name" | "description" | "state">;

const toTeamProjectResult = (project: TeamProjectReference): TeamProjectResult => {
  return {
    id: project.id,
    name: project.name,
...
function configureCoreTools(server: McpServer, tokenProvider: () => Promise<AccessToken>, connectionProvider: () => Promise<WebApi>, disabledTools: Set<string>) {
  if (!disabledTools.has(CORE_TOOLS.list_azure_devops_teams_by_project)) {
    server.tool(
      CORE_TOOLS.list_azure_devops_teams_by_project,
      "[Azure DevOps] Retrieve a list of teams for the specified Azure DevOps project.",
      {
  ...
  if (!disabledTools.has(CORE_TOOLS.list_azure_devops_projects)) {
    server.tool(
      CORE_TOOLS.list_azure_devops_projects,
      "[Azure DevOps] Retrieve a list of projects in your Azure DevOps organization.",
      {
      ...
      async ({ stateFilter, top, skip, continuationToken, projectNameFilter }) => {
        try {
          ...
          const filteredProjects = projectNameFilter ? filterProjectsByName(projects, projectNameFilter) : projects;
          const res = filteredProjects.map((project) => toTeamProjectResult(project));
  ...
  if (!disabledTools.has(CORE_TOOLS.get_azure_devops_identity_ids)) {
    server.tool(
      CORE_TOOLS.get_azure_devops_identity_ids,
      "[Azure DevOps] Retrieve Azure DevOps identity IDs for a provided search filter.",
      {
