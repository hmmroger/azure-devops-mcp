...
function configureBuildTools(server: McpServer, tokenProvider: () => Promise<AccessToken>, connectionProvider: () => Promise<WebApi>, disabledTools: Set<string>) {
  if (!disabledTools.has(BUILD_TOOLS.get_definitions)) {
    server.tool(
  ...
  if (!disabledTools.has(BUILD_TOOLS.get_definition_revisions)) {
    server.tool(
  ...
  if (!disabledTools.has(BUILD_TOOLS.get_builds)) {
    server.tool(
  ...
  if (!disabledTools.has(BUILD_TOOLS.get_log)) {
    server.tool(
      BUILD_TOOLS.get_log,
  ...
  if (!disabledTools.has(BUILD_TOOLS.get_log_by_id)) {
    server.tool(
  ...
  if (!disabledTools.has(BUILD_TOOLS.get_changes)) {
    server.tool(
  ...
  if (!disabledTools.has(BUILD_TOOLS.run_build)) {
    server.tool(
  ...
  if (!disabledTools.has(BUILD_TOOLS.get_status)) {
    server.tool(
  ...
  if (!disabledTools.has(BUILD_TOOLS.update_build_stage)) {
    server.tool(
