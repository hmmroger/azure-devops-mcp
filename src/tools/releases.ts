...
function configureReleaseTools(server: McpServer, tokenProvider: () => Promise<AccessToken>, connectionProvider: () => Promise<WebApi>, disabledTools: Set<string>) {
  if (!disabledTools.has(RELEASE_TOOLS.get_release_definitions)) {
    server.tool(
  ...
  if (!disabledTools.has(RELEASE_TOOLS.get_releases)) {
    server.tool(
