...
function configureTestPlanTools(server: McpServer, tokenProvider: () => Promise<AccessToken>, connectionProvider: () => Promise<WebApi>, disabledTools: Set<string>) {
  /*
    LIST OF TEST PLANS
    get list of test plans by project
  */
  if (!disabledTools.has(Test_Plan_Tools.list_test_plans)) {
    server.tool(
  ...
  */
  if (!disabledTools.has(Test_Plan_Tools.create_test_plan)) {
    server.tool(
  ...
  */
  if (!disabledTools.has(Test_Plan_Tools.add_test_cases_to_suite)) {
    server.tool(
  ...
  */
  if (!disabledTools.has(Test_Plan_Tools.create_test_case)) {
    server.tool(
  ...
  */
  if (!disabledTools.has(Test_Plan_Tools.list_test_cases)) {
    server.tool(
  ...
  */
  if (!disabledTools.has(Test_Plan_Tools.test_results_from_build_id)) {
    server.tool(
