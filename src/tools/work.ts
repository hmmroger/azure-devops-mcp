...
function configureWorkTools(server: McpServer, tokenProvider: () => Promise<AccessToken>, connectionProvider: () => Promise<WebApi>, disabledTools: Set<string>) {
  if (!disabledTools.has(WORK_TOOLS.list_team_iterations)) {
    server.tool(
  ...
  if (!disabledTools.has(WORK_TOOLS.create_iterations)) {
    server.tool(
  ...
  if (!disabledTools.has(WORK_TOOLS.assign_iterations)) {
    server.tool(
